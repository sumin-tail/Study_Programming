...
void BitMap::Draw(HDC hdc, int x, int y, int width, int height) //가로세로 사이즈 변경
{
	TransparentBlt(hdc, x, y, width, height, MemDC, 0, 0, m_Size.cx, m_Size.cy, RGB(255, 0, 255));// 이미지 드로우 - 투명화적용
}

void BitMap::Draw(HDC hdc, int x, int y) //사이즈변경 x
{
	TransparentBlt(hdc, x, y, m_Size.cx, m_Size.cy, MemDC, 0, 0, m_Size.cx, m_Size.cy, RGB(255, 0, 255));// 이미지 드로우 - 투명화적용
}
