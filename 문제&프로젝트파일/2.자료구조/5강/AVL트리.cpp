...
//AVL트리 만들며 새로 작성한 함수
int DepthCheck(Node* node); //깊이 체크
int Hight(Node* node); //높이체크
Node* AVL_Set(Node* node);
...
int DepthCheck(Node* node) //깊이 체크
{
	int leftDepth = Hight(node->left); //왼쪽 깊이 체크
	int rightDepth= Hight(node->right); //오른쪽 깊이 체크
	return leftDepth - rightDepth; //왼쪽에서 오른쪽 깊이를 뺌
}

int Hight(Node* node)//높이 체크
{
	if (node !=NULL) //현재노드가 NULL이 아니라면
	{
		int leftCount = Hight(node->left);
		int rightCount = Hight(node->right);

		if (leftCount > rightCount) //왼쪽의 카운트가 오른쪽 보다 클 경우
		{
			return leftCount + 1; // 왼쪽 카운트 + 1 (자신도 세야 하기 때문에)
		}
		else //아니라면
		{
			return rightCount + 1; 
	...
	else //NULL 노드에 도달했을 경우 0을 들고 되돌아감
	{
		return 0;
	}
}
...
{
	//RR
	Node* temp = node->right; //LL과 작동원리 같음
...
{
	//LL
	Node* temp = node->left; //왼쪽 자식을 내자리로 올려야 하니 세이브 
	node->left = temp->right; //내 왼쪽 자식의 오른쪽 자식을 (오른쪽 자식 또는 NULL이 들어감) 내 왼쪽에 넣음 > 왼쪽 자식과 연결 끊김
	if (node->left != NULL)//만약 오른쪽 자식이 있었다면
	{
		node->left->parent = node; //나를 대체할 노드의 왼쪽 자식이였던 노드의 부모를 나로 갱신 시킴
	}
	temp->right = node; //나를 대체할 노드의 오른쪽에 나를 넣음
	temp->parent = node->parent; //나를 대체할 노드의 부모를 나의 부모로 재설정
	node->parent = temp; //나의 부모를 나를 대체할 노드로 변경
	return temp;
}
...
Node* AVL_Set(Node* node)
{
	...
	if (Depth >= 2) // 왼쪽이 더 높음
	{
		if (DepthCheck(node->left) >=1)//자식노드 검사
		{
			//LL
	...
	else if (Depth <= -2)//오른쪽이 더 높음
	{
		if (DepthCheck(node->right) <= -1)
		{
			//RR
