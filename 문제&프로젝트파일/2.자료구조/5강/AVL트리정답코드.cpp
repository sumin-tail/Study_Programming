...
Node* AVLSet(Node* node)
{
	...
	if (Depth > 1)
	{
		if (DepthCheck(node->left) > 0) //여긴 LL -> LR 순이지만
			node = LL(node); //깊이가 양수라면 LL이지만
		else
			node = LR(node); //아니라면 LR임
	}
	else if (Depth < -1)
	{
		if (DepthCheck(node->right) > 0) //여긴 RL -> RR 순 > 위쪽과 조건이 같음 > 조건이 같은 대신 순서가 다름
			node = RL(node); // RL이 되려면 양수여야함
		else
			node = RR(node); // RR이 되려면 DepthCheck(node->right) 가 0 이하여야 함
	}
	return node;
}
